...
import io
import csv
from core import gcp_services

# --- Tests para la autenticación y el servicio ---


@patch("core.gcp_services.service_account")
@patch("core.gcp_services.build")
def test_get_google_service(mock_build, mock_service_account):
    ...
    scopes = ["scope1"]
    service = gcp_services.get_google_service(
...
def test_find_file_on_drive_found(mock_google_service):
    ...
    result = gcp_services.find_file_on_drive(
        mock_google_service, "file1.txt", "folder123"
    )
...
@patch("core.gcp_services.MediaIoBaseDownload")
@patch("core.gcp_services.io.BytesIO")
def test_download_csv_from_drive(mock_bytesio, mock_download, mock_google_service):
    ...
    result = gcp_services.download_csv_from_drive(mock_google_service, mock_file_id)
    assert result == mock_content.decode("utf-8")


@patch("core.gcp_services.download_csv_from_drive")
def test_download_csv_as_dict_success(mock_download_csv):
    ...
    result = gcp_services.download_csv_as_dict(None, "file_id")
    assert len(result) == 2
...
def test_download_csv_as_dict_no_file_id():
    ...
    with pytest.raises(FileNotFoundError):
        gcp_services.download_csv_as_dict(None, None)


@patch("core.gcp_services.MediaIoBaseUpload")
def test_upload_csv_to_drive_update(mock_upload, mock_google_service, capsys):
    ...
    gcp_services.upload_csv_to_drive(
...
def test_get_sheets_data_success(mock_sheets_service):
    ...
    result = gcp_services.get_sheets_data(mock_sheets_service, "spreadsheet_id")
